...
await check('minting wins over a landing-URL token', async () => {
  // The landing token lives 30 seconds and the web app spends it as the page
  // loads, so a minted token is the only one worth delivering when both exist.
